...
export default function BlogCard({ post, featured }: BlogCardProps) {
  if (featured) {
    return (
      ...
      >
        <Link to={`/blog/${post.slug}`} className="group block glass-card-hover overflow-hidden">
  ...
  return (
    ...
    >
      <Link to={`/blog/${post.slug}`} className="group block glass-card-hover overflow-hidden h-full">
